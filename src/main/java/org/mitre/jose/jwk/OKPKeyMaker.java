/*
 * Copyright 2020 The MITRE Corporation and
...
public class OKPKeyMaker {
	...
	public static JWK make(Curve keyCurve, KeyUse keyUse, Algorithm keyAlg, String kid) {
		try {
			...
			// Java only gives us the keys in ASN.1 format so we need to parse
			// them back out to get the raw numbers
			...
			// For some reason, sometimes the private key is double-wrapped in
			// OctetStrings and we need to unpack that.
			...
			return new OctetKeyPair.Builder(keyCurve, Base64URL.encode(x))
